...
const RailButton: React.FC<{
  ...
  active?: boolean;
  darkMode: boolean;
  theme?: 'blue' | 'amber' | 'emerald' | 'purple' | 'indigo' | 'fuchsia' | 'teal' | 'slate';
}> = ({ icon, label, onClick, active, darkMode, theme = 'blue' }) => {
  const getThemeColors = (t: string) => {
    switch (t) {
      case 'blue': return { text: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500' };
      case 'amber': return { text: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500' };
      case 'emerald': return { text: 'text-emerald-500', bg: 'bg-emerald-500/10', border: 'border-emerald-500' };
      case 'purple': return { text: 'text-purple-500', bg: 'bg-purple-500/10', border: 'border-purple-500' };
      case 'indigo': return { text: 'text-indigo-500', bg: 'bg-indigo-500/10', border: 'border-indigo-500' };
      case 'fuchsia': return { text: 'text-fuchsia-500', bg: 'bg-fuchsia-500/10', border: 'border-fuchsia-500' };
      case 'teal': return { text: 'text-teal-500', bg: 'bg-teal-500/10', border: 'border-teal-500' };
      case 'slate': return { text: 'text-slate-500', bg: 'bg-slate-500/10', border: 'border-slate-500' };
      default: return { text: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500' };
    }
  };

  const colors = getThemeColors(theme);
  const activeClass = `${colors.bg} ${colors.text} border-r-4 ${colors.border} font-bold`;
  ...
  return (
    ...
    >
      ...
      {/* Tooltip */}
      <div className={`absolute left-full ml-2 px-2.5 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none duration-150 shadow-md border z-50
        ${darkMode ? 'bg-zinc-900 text-white border-zinc-800' : 'bg-white text-gray-800 border-gray-200'}`}
      >
        {label}
      </div>
    </button>
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  onSave, darkMode, toggleDarkMode, onShowHistory, onShowSettings, onShowStats, onRemoveFromHistory, uiLanguage, onShowCollaboration
}) => {
  ...
  return (
    <div className="flex h-full z-50 shrink-0 relative">
        {/* Permanent Rail */}
        <div className={`w-20 flex flex-col items-center py-6 space-y-5 z-20 relative ${railBg}`}>
           <div className="mb-2">
              <button 
                 type="button"
           ...
           <RailButton
             icon={<Users size={24} />}
             label={t(uiLanguage, 'collaborate')}
            ...
            <div className="w-64 flex flex-col h-full"> {/* Inner container to prevent squishing */}
                <div className="p-5 border-b border-gray-200/10">
                   ...
                   <div className="flex items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 text-[10px] font-semibold rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">BETA</span>
                    </div>
